...
public class NumericTextField<T extends Number & Comparable<T>>  extends JFormattedTextField {
	...
	public void setConstraits(final T min, final T max) {
	...
	public Action upAction() {
		return new AbstractAction() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				} else if(((InternationalFormatter)ff.getEditFormatter()).getMaximum().compareTo(getNumber()) >= 0) {
	...
	public Action downAction() {
		return new AbstractAction() {
			...
			public void actionPerformed(ActionEvent e) {
				final DefaultFormatterFactory ff = (DefaultFormatterFactory)getFormatterFactory();
				if(isEmpty() == false && ((InternationalFormatter)ff.getEditFormatter()).getMinimum().compareTo(getNumber()) < 0) {
					final Number val = getNumber();
					if(val instanceof Float) {
						NumericTextField.this.setValue(val.floatValue() - 1);
					} else if (val instanceof Double) {
